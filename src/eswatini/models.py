...
class Geocollection(models.Model):
    ...
    def get_users_with_perms(self):
        ...
        for perm in Permission.objects.filter(
            codename="access_geocollection", content_type_id=ctype.id
        ):
            permissions[perm.id] = perm.codename

        user_model = get_user_obj_perms_model(self)
        users_with_perms = user_model.objects.filter(
            object_pk=self.pk,
        ...
        profiles = {}
        for profile in get_user_model().objects \
                .filter(id__in=list(users.keys())):
    ...
    def set_default_permissions(self):
        ...
        # assign permissions to 'Anyone'
        # if 'DEFAULT_ANONYMOUS_VIEW_PERMISSION' is True
                    ...
                    if managers:
                        for manager in managers:
                            if (
                                ...
                                and not manager.is_superuser
                                # and manager != resource.owner
                            ):
                                assign_perm(
                                    "access_geocollection", manager, resource
                                )
                                users[manager] = [
